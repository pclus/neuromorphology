...
#define INHI2INHI (in2in*0.7) // 0.8
#define EXCI2INHI (2.1)

#define ALPHAexci (0.008/(TAUexci-RAUexci))
//#define ALPHAexci (0.008/(TAUexci-RAUexci))
#define ALPHAinhi (0.240/(TAUinhi-RAUinhi))
//#define ALPHAinhi (0.320/(TAUinhi-RAUinhi))
#define ALPHAmini (0.012/(TAUmini-RAUmini))
//#define ALPHAmini (0.012/(TAUmini-RAUmini))
//
//#define INHI2INHI (in2in*0.7) // 0.8

//#define ALPHAexci (0.008/(TAUexci-RAUexci))
//#define ALPHAinhi (0.220/(TAUinhi-RAUinhi))
//#define ALPHAmini (0.012/(TAUmini-RAUmini))
//
...
#define TAUexci (2.0)
#define TAUinhi (4.0)
#define TAUmini (TAUexci)

#define RAUexci (0.5)
#define RAUinhi (2.0)
#define RAUmini (RAUexci)

#define STRENGTH (1.0)

#define EXCI_INPUT(t) (ALPHAexci*exp(-(t)/TAUexci))
#define INHI_INPUT(t) (ALPHAinhi*exp(-(t)/TAUinhi))
#define MINI_INPUT(t) (ALPHAmini*exp(-(t)/TAUmini))
#define EXCI_INIT_INPUT(t) (exp(-(t)/TAUexci))
#define INHI_INIT_INPUT(t) (exp(-(t)/TAUinhi))
#define MINI_INIT_INPUT(t) (exp(-(t)/TAUmini))

#define EXCI_RAISEINPUT(t) (ALPHAexci*exp(-(t)/RAUexci))
#define INHI_RAISEINPUT(t) (ALPHAinhi*exp(-(t)/RAUinhi))
#define MINI_RAISEINPUT(t) (ALPHAmini*exp(-(t)/RAUmini))
#define EXCI_INIT_RAISEINPUT(t) (exp(-(t)/RAUexci))
#define INHI_INIT_RAISEINPUT(t) (exp(-(t)/RAUinhi))
#define MINI_INIT_RAISEINPUT(t) (exp(-(t)/RAUmini))
...
//	int flag=atoi((argv[6]));
	...
	Routputs=(double *)malloc(sizeof(double)*N);
	external=(bool *)malloc(sizeof(bool)*N);
	lfp=(bool *)malloc(sizeof(bool)*N);
	

	Rexci=(double *)malloc(sizeof(double)*N);		
	Rinhi=(double *)malloc(sizeof(double)*N);	
	Rmini=(double *)malloc(sizeof(double)*N);
	...
	excitatory_raisecurrent=EXCI_INIT_RAISEINPUT(dt);
	inhibitory_raisecurrent=INHI_INIT_RAISEINPUT(dt);
	synaptic_raiseminicurrent=MINI_INIT_RAISEINPUT(dt);
	
	//top=ALPHAexci/TAUexci;
	//bottom=ALPHAinhi/TAUinhi;
	top=ALPHAexci*(pow(RAUexci/TAUexci,RAUexci/(TAUexci-RAUexci))-pow(RAUexci/TAUexci,TAUexci/(TAUexci-RAUexci)));
	bottom=ALPHAinhi*(pow(RAUinhi/TAUinhi,RAUinhi/(TAUinhi-RAUinhi))-pow(RAUinhi/TAUinhi,TAUinhi/(TAUinhi-RAUinhi)));

	fprintf(stderr,"topbottom: \n%lf\n%lf\n",top,bottom);
...
int initial_conditions(double ampl){
	...
	for(i=0;i<N;i++){
		...
		Rexci[i]=Rinhi[i]=Rmini[i]=0;
		//external[i]=(RAND < 0.2 ? true : false);
		//external[i]=( sqrt( (coordx[i]-750)*(coordx[i]-750)+(coordy[i]-750)*(coordy[i]-750)) <300 && inhibitory[i]==1 ? true : false);
...
int euler(){
	...
	synapses(t);
        for(i=0;i<N;i++){
                ...
                k=f(x[i],y[i],(Iexci[i]+Imini[i]-Rexci[i]-Rmini[i])*(EE-x[i])-(Iinhi[i]-Rinhi[i])*(EI-x[i])+IC[i]);  // using reversal potential
		...
		if(inhibitory[i]==1)
			activity+=fabs((Iexci[i]+Imini[i]-Rexci[i]-Rmini[i])*(EE-x[i]))+fabs((Iinhi[i]-Rinhi[i])*(EI-x[i]));
                new_spyke[i]=spyke_reseting(i);
        }
	//activity/=(1.*Nex);
...
int spyke_reseting(int i){
	...
	bool inhi=(inhibitory[i]==-1);
                ...
                if(inhi)
		{
                          ...
                          raise=-INHI_RAISEINPUT(t-ts)/inhibitory_raisecurrent;
		}
                else
		{
                          current=EXCI_INPUT(t-ts)/excitatory_current;
                          raise=EXCI_RAISEINPUT(t-ts)/excitatory_raisecurrent;
		}
                outputs[i]+=current;
                Routputs[i]+=raise;
		//----------------------------------------------
...
void synapses(double t){
	int i,j,jj;
	bool inhi;
	double w,current,ts,raisecurrent;
	...
	for(i=0;i<N;i++){
		...
		if(ts > 0){
			if(inhi)
			{
				...
				raisecurrent=-INHI_RAISEINPUT(t-ts-1)/inhibitory_raisecurrent;
			}
			else
			{
				current=EXCI_INPUT(t-ts-1)/excitatory_current;
				raisecurrent=EXCI_RAISEINPUT(t-ts-1)/excitatory_raisecurrent;
			}
			...
			for(j=cumulative_degree[i];j<cumulative_degree[i+1];j++){
                        	...
                        	if(inhi)
				{
					if(inhibitory[jj]==-1)
					{
						...
						Rinhi[jj]+=INHI2INHI*w*raisecurrent;
						//Iinhi[jj]+=INHI2INHI*w*current;
					}
                        	        else
					{
						Iinhi[jj]+=w*current;
						Rinhi[jj]+=w*raisecurrent;
					}
				}	
                        	else
				{
					if(inhibitory[jj]==-1)
					{
                        	        	Iexci[jj]+=EXCI2INHI*w*current;
                        	        	Rexci[jj]+=EXCI2INHI*w*raisecurrent;
					}
					else
					{
                        	        	Iexci[jj]+=w*current;
                        	        	Rexci[jj]+=w*raisecurrent;
					}
				}
                	}
		}
		...
		if(external[i])
			if(shot_noise()){
				...
				Rmini[i]+=MINI_RAISEINPUT(t-ts)/synaptic_raiseminicurrent;
		}
	}
	...
	for(i=0;i<N;i++){
		...
		Rexci[i]*=excitatory_raisecurrent;
		Rinhi[i]*=inhibitory_raisecurrent;
		Rmini[i]*=synaptic_raiseminicurrent;
		if(inhibitory[i]==-1)
		{
			outputs[i]*=inhibitory_current; 
			Routputs[i]*=inhibitory_raisecurrent; 
		}
		else
		{
			outputs[i]*=excitatory_current;
			Routputs[i]*=excitatory_raisecurrent;
		}
	}
	return ;
}
